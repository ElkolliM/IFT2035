...
check(Env, if(E, Et, Ee), T) :- check(Env, E, bool), check(Env, Et, T), check(Env, Ee, T).
check(Env, fn(X,E), (T1 -> T2)) :- check([(X, T1, _)|Env], E, T2).
check(Env, app(E1, E2), T2) :- check(Env, E1, (T1 -> T2)), check(Env, E2, T1).
check(Env, tfn(A, E), forall(A, T1)) :-  check([(A, type, _)|Env], E, T2), check_type(T1,T2).
check(Env, tapp(E, T1), T) 
        :- check(Env, E, forall(A, T2)), 
           subst(T2, A, T1, T).
check(Env, list(T), type) :- check(Env, T, type).
check(Env, (T1 -> T2), type) :- check(Env, T1, type), check(Env, T2, type).
check(Env, forall(A, T), type) :- check([(A, type, _)|Env], T, type).
check(_, macro(_), macroexpander).

%% test relation (to delete)
checkit(Env, E, T) :- env0(Env), check(Env, E, T).
...
curry(_, [], R, R).
curry(Env, [Head | Tail], F, R) 
    :- elaborate(Env, Head, HeadE),
       curry(Env, Tail, app(F, HeadE), R).

elaborate(Env, define(X,E), define(X,Ee)) :- elaborate(Env, E, Ee). %% usefull??
elaborate(_, T : Decl, T : Decl). %% declarations are already elaborated.

    %% application de type sont implicites! => app(tfn(X,E), A) donne app(tapp(tfn(X,E), ?), A).
    %% cas: on a nous meme ecrit tfn(_,_). 
elaborate(Env, app(tfn(X,E), A), R)
      :- elaborate(Env, app(tapp(tfn(X, E), _), A), R).  %% polymorphisme implicite

elaborate(_, tapp(E1, E2), tapp(E1, E2)).

elaborate(_, quote(E), quote(E)).
elaborate(Env, app(F,A), app(Fe, Ae)) %% recursion!
      :- elaborate(Env, F, Fe),
         elaborate(Env, A, Ae).
elaborate(Env, fn(X,E), fn(X,Ee))     %% recursion!
      :- elaborate(Env, E, Ee).

elaborate(Env, macro(M), R)        %% macro: TODO
      :- elaborate(Env, M, Me), 
      eval(Env, Me, R).            %% elaborate -> eval -> elaborate ?


%% lorsqu'une variable definie dans l'environemetn a type polymorphique forall(_,_), 
%% => tapp implicite!
%%      app(identity, A) donne app(tapp(identity, ?), A)
%%      ""  nil, A
%%          cons, A
%%          nimporte quelle variable de type forall(_,_).

%% TODO: map : forall(_,forall(_,_)) mais on n'ajoute qu'un seul tapp!
%   => recursion sur lookup(Env,E, forall(_,forall(_,_),_))
%
elaborate(Env, E, R)               %% polymorphisme implicite!
      :- atom(E), lookup(Env, E, forall(_,_),_), R = tapp(E, _).

  %% currying
  %% fun(x,y,z) => app(app(app(fun, x), y, z))
elaborate(Env, E, R) 
    :- E =.. [Fun | [Arg | Args]],
       elaborate(Env, Fun, FunE),
       elaborate(Env, Arg, ArgE),
       curry(Env, Args, app(FunE, ArgE), R).
elaborate(_, E, E).
...
%         Probleme: les applications de type ajoutée par elaborate sont limité a type -> type,
%         map est de type type -> type -> type
%
